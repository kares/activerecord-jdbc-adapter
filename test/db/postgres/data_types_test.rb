...
class PostgresqlDataTypeTest < Test::Unit::TestCase
  ...
  class PostgresqlLtree < ActiveRecord::Base; end
...
_SQL
  ...
  #puts "HSTORE: #{connection.select_value("select 'hstore'=ANY(select typname from pg_type)")}"
  
  if 't' == connection.select_value("select 'hstore'=ANY(select typname from pg_type)")
  execute <<_SQL
...
_SQL
  ...
  #puts "LTREE: #{connection.select_value("select 'ltree'=ANY(select typname from pg_type)")}"
  
  if 't' == connection.select_value("select 'ltree'=ANY(select typname from pg_type)")
  execute <<_SQL
...
_SQL
  ...
  #puts "JSON: #{connection.select_value("select 'json'=ANY(select typname from pg_type)")}"
  
  if 't' == connection.select_value("select 'json'=ANY(select typname from pg_type)")
  execute <<_SQL
...
postgresql_hstores postgresql_ltrees postgresql_json_data_type postgresql_moneys 
postgresql_numbers  postgresql_times postgresql_network_addresses postgresql_bit_strings
postgresql_oids postgresql_timestamp_with_zones).each do |table_name|
